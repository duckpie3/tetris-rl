    ...
    def render(self, fps=FPS):
        ...
        if self.figure:
            for i in range(4):
                for j in range(4):
                    if i * 4 + j in self.figure.image():
                        img = self.Assets[self.figure.color]
                        x = CELLSIZE * (self.figure.x + j)
                        y = CELLSIZE * (self.figure.y + i)
                        self.win.blit(img, (x, y))
                        pygame.draw.rect(self.win, WHITE, (x, y, CELLSIZE, CELLSIZE), 1)

        ghost_cells = self.project_landing()
        ...
        if self.gameover:
            ...
            msg1 = self.font2.render("Press r to restart", True, RED)
            msg2 = self.font2.render("Press q to quit", True, RED)

            self.win.blit(over, (rect.centerx - over.get_width() / 2, rect.y + 20))
            self.win.blit(msg1, (rect.centerx - msg1.get_width() / 2, rect.y + 80))
            self.win.blit(msg2, (rect.centerx - msg2.get_width() / 2, rect.y + 110))
        ...
        scoreimg = self.font.render(f"{self.score}", True, WHITE)
